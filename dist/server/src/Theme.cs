...
namespace NtoNServerControl
{
    ...
    public class GButton : Button
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            g.FillRectangle(new SolidBrush(t.CardAlt), 0, 0, Width, Height);
            using (var path = RoundedPanel.RoundedRect(rect, Radius))
            {
                if (Enabled)
                {
                    ...
                    {
                        g.FillPath(b, path);
                        if (_hover) g.FillPath(new SolidBrush(Color.FromArgb(22, Color.White)), path);
                        if (_pressed) g.FillPath(new SolidBrush(Color.FromArgb(34, Color.Black)), path);
                    }
                }
                else
                {
    ...
    public class NavItem : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var g = e.Graphics;
            g.SmoothingMode = SmoothingMode.AntiAlias;
            g.FillRectangle(new SolidBrush(t.SidebarBg), 0, 0, Width, Height);
            var row = new Rectangle(8, 2, Width - 16, Height - 4);
            if (_hover || Active)
            {
                using (var b = new SolidBrush(Active ? Theme.AccentTint() : t.Hover))
            ...
            if (Active)
            {
                ...
                using (var path = RoundedPanel.RoundedRect(new Rectangle(4, (Height - 16) / 2, 3, 16), 2)) g.FillPath(b, path);
            }
            TextRenderer.DrawText(g, Text, Font, new Rectangle(24, 0, Width - 32, Height),
